import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0" # Ensure you GPU have atleast 8 GB VRAM
os.environ["TRITON_CACHE_DIR"] = "./triton_cache_gpu"
import json
import time
import glob
import copy
...
def main(config):
    ...
    # Check for AMP
    logger.info(f"AMP ENABLE: {config.AMP_ENABLE}")
    ...
    if QUANTIZE_ENA:
        ...
        models["is_quantized"] = False
        logger.info(f"PERFORM REGULAR TRAINING/EVAL - NO QUANTIZATION!")
    
    # Check AMP
    logger.info(f"AMP MODE: {config.AMP_ENABLE}")
    
    # Set the models to device    
    for k, v in models.items():
        if k != "is_quantized":
            models[k] = v.to(device)
    ...
    # Evaluation mode
        ...
        if QUANTIZE_ENA:
            logger.info("Checking if fp8 model exist...")            
        else:
            logger.info("Load regular fp32 model...")
            models = {"generator": models["generator"]}
        
        data_loader_test = get_loader(config, logger)
        logger.info(f"TESTING: ({len(data_loader_test)} files)")
        
        # Set device and quantize backend
        if QUANTIZE_ENA:
            device = "cpu"
            device_ids = 0
        ...
        tester = Tester(
            ...
            config=config,
            # device=(device, device_ids),
            device=(device, device_ids) if not QUANTIZE_ENA else "cpu",
    ...
    # Training mode
        ...
        # trainer.train(is_quantized=QUANTIZE_ENA)
        trainer.train()
