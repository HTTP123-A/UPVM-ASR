...
def load_from_path(models, optimizer, config, logger):
    ...
    if not config.EVAL_MODE and not config.INFERENCE_MODE:
        ...
        if len(checkpoint_files) == 0:
            ...
            return (models, optimizer, config, start_epoch, is_quantized)
        else:
            for file in checkpoint_files:
                try:
                    checkpoint = torch.load(file)
                    # Get quantization flag
                    is_quantized = checkpoint["is_quantized"]
                    # Get the model type
                    model_type = file.split("-")[2].split(".")[0]
                    key = "generator" if model_type == "G" else model_type
                    # Load the model - fake quant (still fp32)
        ...
        if len(checkpoint_files) == 0:
            ...
            try:
                ...
                if is_quantized:
                    logger.info("Loading quantize model...")
                    if 'state_dict_int8' in checkpoint:
                        logger.info("Found quantize model in saved state_dict")
                        models["generator"].load_state_dict(checkpoint["state_dict_int8"])
                    else:
                        raise ValueError("Cannot find key `state_dict_int8` in checkpoint!")
                    
                    logger.info(f"Loaded quantized generator from {checkpoint_files[0]}")
                else:
                    logger.info("Loading fp32 model...")
                    models["generator"].load_state_dict(checkpoint["state_dict"])
                    # Log the loaded model
                    logger.info(f"Loaded fp32 generator from {checkpoint_files[0]}")
            except:
                logger.error(
