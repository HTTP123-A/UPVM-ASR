...
def wav2spectro(
    ...
    batch, channel, length = waveform.shape
    
    window = torch.hann_window(win_length)
    window = window.to(waveform)
    ...
    mag = torch.log2(torch.abs(spec) + 1e-8) # In VM-ASR, log2 is used
    phase = torch.angle(spec)

    # return mag.view(*other, freqs, frame), phase.view(*other, freqs, frame)
    return mag.view(batch, channel, freqs, frame), phase.view(batch, channel, freqs, frame)
...
def spectro2wav(
    ...
    """
    ISTFT from magnitude/phase without Python complex literals (export-safe).
    mag, phase: (..., F, T) with real tensors
    """
    device = mag.device
    dtype  = mag.dtype

    # window must match device/dtype used by istft
    # window = torch.hann_window(win_length, device=device, dtype=dtype)
    window = torch.hann_window(win_length)
    window = window.to(mag)
    ...
    # n_fft_calc = 2 * F - 2   # consistent with your STFT layout
    n_fft_calc = 1024

    mag = mag.view(-1, F, T)
    phase = phase.view(-1, F, T)
    
    amp = torch.exp2(mag)

    # Build complex spectrum: amp * (cos + i sin), no `1j` anywhere
    spec = torch.polar(amp, phase)  # complex tensor
    ...
    # return wav.view(*other, L)
    return wav.reshape(B, C, L)


# -------------------------------------------------------------------------------
def wav2spectro_flex(
    waveform: torch.Tensor,
    n_fft: int,
    hop_length: int,
    win_length: int,
