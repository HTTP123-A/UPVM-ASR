...
from utils.stft import wav2spectro_flex
...
def log_waveform(writer, wave_in, wave_out, wave_target, config):
    ...
    try:
        ...
        # Skip entirely if anything goes wrong
        return
...
def fig2np(fig):
    ...
    try:
        fig.canvas.draw()
        data = np.asarray(fig.canvas.buffer_rgba())  # Matplotlib 3.9+
