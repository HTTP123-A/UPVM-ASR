...
from utils.visqol_cli import compute_folder_average_visqol
from utils.whisper_asr import compute_folder_whisper_cer_wer
...
class Tester(BaseTester):
    ...
    def __init__(
        ...
        if self.is_quantized:
            if self.device == "cpu":
                logger.info(f"Model quantized mode {self.is_quantized} and run on {device}")
            else:
                raise ValueError(f"Quantized model cannot run on {device}")
        else:
            logger.info(f"Model running in floating mode!")

        for name, mod in self.models["generator"].named_modules():
            if "quantized" in mod.__class__.__name__.lower():
                print(f"[INT8] {name}: {mod.__class__.__name__}")
            ...
            with logging_redirect_tqdm(loggers=[self.logger], tqdm_class=tqdm):
                with tqdm(
                    ...
                    for batch_idx, (
                        ...
                        highcut = highcut.to(self.device)
                ...
                # Compute parallel ViSQOL across saved files
                self.logger.info("Starting parallel ViSQOL evaluation...")
                visqol_avg = compute_folder_average_visqol(self.output_dir, num_workers=100)
                self.logger.info(f"Average ViSQOL MOS-LQO over test set: {visqol_avg:.4f}")
                self.test_log["visqol"] = visqol_avg
                
                # --- Whisper CER/WER using *_orig.wav / *_up.wav ---
                asr_res = compute_folder_whisper_cer_wer(
                    folder=self.output_dir,
                    vctk_root="data/VCTK-Corpus-0.92",   # adjust
                    model_name="large",
                )
                self.test_log.update(asr_res)
    ...
    def _evaluate_batch(self, wave_out, wave_target, highcut):
        metrics = {
            metric.__name__: metric(
                wave_out.squeeze(1), wave_target.squeeze(1), hf=highcut, sr=self.target_sr
            )
